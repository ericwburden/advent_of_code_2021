...
# Check whether two cube overlap
...
# below, but it's interesting enought to keep around.
...
# imagine these values as line endpoints on a number line.
...
function shatter(target::Cube, template::Cube)
    ...
    for (a, b, c) in Iterators.product(1:3, 1:3, 1:3)
        a == b && b == c && c == 2 && continue # Skip the `template` range
...
function part2(input)
    ...
    for instrin in input
        ...
        push!(finalinstructions, instrin)
    end
